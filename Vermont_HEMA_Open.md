# Death Pool Longsword Ruleset

*Derived from the Revolution Rumble 2025 rules (Bucks Historical Longsword). Scoring, priority, afterblow, and tournament structure have been rewritten; hit quality, penalties, gear, and court logistics are carried over with minimal changes.*
...
Every valid hit is assigned a point value based on how it was delivered.
...
Each exchange resolves to one of the following:

| Outcome | Result |
| :---- | :---- |
| Clean hit | Hitting fencer scores the full value of their action. |
...
## 1.3: Double Hits vs. Afterblows

This ruleset distinguishes between a *double hit* and an *afterblow*. The distinction is when the second action **started** relative to when the first hit **landed**.

***Double hit*** - Both fencers land valid hits, and the second fencer's action was already in progress (final decision to commit made, blade travelling toward the target) when the first hit landed. Doubles are resolved by priority (Section 1.5).

***Afterblow*** - A valid hit that is *initiated after* the opponent's hit has already landed. Afterblows are never resolved by priority; by definition the initial hit has priority. Instead, a valid afterblow deducts 1 point from the initial hit.

***Afterblow validity requirements***

An afterblow is valid only if **all** of the following are met:

1. It begins after the initial hit lands. (If it was already in progress, it is a double, not an afterblow.)
...
A "valid hit" is a cut or thrust that meets the hit quality requirements below and lands on a valid target (Section 1.6).

***Offensive blade action definitions***
Cut - A percussive action delivered with the edge of the sword
Thrust - A percussive action delivered with the tip of the sword
Slice - An action with the edge of the sword which may include sliding action across the surface or pressure against the target, but no percussive hit

***Hit quality requirements***
...
Priority is used **only** to resolve double hits (Section 1.3). It is never used to evaluate afterblows.

Priority is layered. Compare the two actions one layer at a time, starting from the top. The first layer in which the two actions differ decides priority; lower layers are not consulted.

| Layer | Order of priority (highest first) |
| :---- | :---- |
| **1. Hand** | Two-handed action > One-handed action |
| **2. Target** | Above the arms > Below the arms > Legs (see Section 1.6) |
| **3. Tactic** | Attack > Counterattack > Riposte > Renewal (see Appendix A) |
| **4. Action** | Thrust > Cut |
...
***Valid targets:*** The whole body is a valid target except the back of the head, the spine (see penalties), and the ankle and below (feet). Hits to the ankle or foot are not valid hits and score nothing. The groin is not a cardable target; call excessive force if necessary.

***Target tiers (used at the Target layer of priority):***

Target tiers are **positional, not anatomical**. The question is whether the hitting blade reached its target by going **over** the opponent's arms or **under** them, judged by where the opponent's arms actually are *at the moment the hit lands*.

| Tier | Name | Definition |
| :---- | :---- | :---- |
| 1 | **Above the arms** | The blade reaches the target by passing over the opponent's arms, or lands on the **top** of their forearms, hands, or arms. |
| 2 | **Below the arms** | The blade reaches the target by passing under the opponent's arms, or lands on the **underside** of their forearms, hands, or arms. |
| 3 | **Legs** | A hit that lands below the hip and above the ankle. |
...
- A "grapple" is defined as when a fencer makes purposeful contact with their hand against their opponent's sword or person.
- When grappling is initiated, it will be allowed to continue as long as progress is being made. If both swords are entangled and unable to score, expect halt to be called.
- Scoring actions from a grapple include:
  - Striking with the blade of the sword
...
## 2.1: Cards and Escalation

***Card definitions***

- Yellow card: warning
- Red card: 2 points awarded to opponent
- 2 yellow cards = 1 red card (after a yellow card is issued in a match, all other penalties that would be yellow cards become red cards)
- Black card: disqualification from the tournament
- 3 "safety" red cards = black card (non-safety reds can stack indefinitely; the count of safety reds carries over between matches)

A fencer may not earn points on the same exchange in which they are penalized.
...
***Intentional Contact with Pommel*** - Strike that lands with the pommel on any part of the body.

***Striking to Back of Head*** - Striking with any part of the sword to the back of the opponent's head.
...
***Sword Throwing*** - Throwing the sword or any part of it at the opponent.

***Striking with a body part other than sword to deal percussive damage*** - Punches, elbows, knees, headbutts, etc.
...
- Fencing mask - no dents in mesh, no tears in lining
- Back of head protection
- Purpose built puncture resistant HEMA jacket (minimum 350 Newton)
- Hard elbow guards
- Hard knee guards
- Hard shin guards
- Hard forearm guards
- Hard neck protector
- Purpose-built longsword fencing gloves (no lacrosse gloves or Red Dragons; Thokk only allowed with longsword peripheral attached)
...
## 3.2: Fencing Foil (Feder)

- Must be a purpose built longsword fencing foil (a "feder")
...
- Courts shall be rectangular, approximately 8 meters long and 4 meters wide.
- Starting lines shall be placed 2 meters from the marked center of the court (4 meters apart).

## 4.2: Matches

- Matches are first to 10 points in pools, first to 14 in eliminations.
- Time limit is 2 minutes for pool matches and 3 minutes for elimination matches, paused during stoppages.
- If time runs out during an exchange, the fencers shall be allowed to finish the exchange with no stoppage.
- If the match is tied at the end of regulation time, fencers will fence an unlimited-time tiebreaker in which the first fencer to score any points wins. (A hit reduced to 0 by an afterblow does not end the tiebreaker.)
...
- All fencers are ranked 1 through N by HEMA Ratings. Unrated fencers are placed at the bottom of the ranking (order among unrated fencers is at the organizer's discretion).

***Pool assignment (linear seeding)***

- Pools are of 6 or 7 fencers, chosen so that pool sizes are as even as possible. Aim for pools of 7; use pools of 6 only as needed to balance.
- Pools are filled **linearly**, not serpentine: Pool 1 receives seeds 1 through 7, Pool 2 receives seeds 8 through 14, Pool 3 receives seeds 15 through 21, and so on.
- Every fencer fences every other fencer in their pool once.
...
- Each pool owns a contiguous block of bracket seed positions equal to its size, in pool order. Pool 1 owns bracket seeds 1 through 7, Pool 2 owns seeds 8 through 14, and so on.
- Within that block, seeds are assigned by pool ranking: the winner of Pool 1 is bracket seed 1, the runner-up of Pool 1 is bracket seed 2, ..., the 7th place finisher in Pool 1 is bracket seed 7; the winner of Pool 2 is bracket seed 8, and so on.
- A fencer can never leave their pool's seed block regardless of pool results. Pool results only reorder fencers *within* their block.
- All fencers advance to the bracket. There are no cuts after pools.
...
| Pool | HEMA Rating seeds | Bracket seeds owned |
| :---- | :---- | :---- |
| Pool 1 | 1-7 | 1-7 |
| Pool 2 | 8-14 | 8-14 |
| Pool 3 | 15-21 | 15-21 |

If seed 5 wins Pool 1, they become bracket seed 1. If seed 1 finishes last in Pool 1, they become bracket seed 7, still ahead of everyone from Pool 2.

---

# Appendix A: Detailed Tactical Definitions for Priority Convention

*Note: "Priority" is a property of an action, not a fencer. If fencer A attacks fencer B, it is more appropriate to say "fencer A's action has priority" than "fencer A has priority."*

*These definitions govern the **Tactic** layer of priority (Section 1.5) and the **chambering** requirement for afterblows (Section 1.3).*
...
- An attack will be reset via a "chambering" action. If after the fencer has started moving forward the sword moves backwards during the attacking action, that shows that the action was not the fencer's final decision to commit, and the subsequent action will be considered a renewal.
  ...
  - If a fencer re-chambers during their final approach to attack, they have given up their attack priority, and the following action will be considered a renewal. Example: fencer on the right attacks with a cut from the right, pulls back before their cut lands (feints), and attacks the left side landing a hit:
    - R feint -> L hits -> R hits = Attack right no, counterattack left yes, renewal right yes (point Left)
    - R feint -> L and R hit simultaneously = Attack right no, counterattack left yes, renewal right yes (point Left)
    - R feint -> R hits -> L starts after = Attack right yes; L's hit is evaluated as an afterblow (Section 1.3), not a double
  - The blade does not need to go directly straight to the target to maintain priority; it can travel in a curved path, as long as it does not go backwards.
...
- Calls should be made as quickly as possible; if you are sure about a call, make it right away.
- Announce calls in the order the rules evaluate them: **(1)** which hits landed and were quality, **(2)** whether the exchange is a clean hit, a double, or a hit-plus-afterblow, **(3)** for a double, which priority layer decided it, **(4)** for an afterblow, whether it was valid and why, **(5)** the point value awarded.
  - Example: "Two hits. Double. Both two-handed, both to the head. Attack from the right, counterattack from the left. Two points right."
  - Example: "Hit right, then afterblow left. Afterblow chambered, too late. Two points right."
  - Example: "Hit right, afterblow left, in time. Two minus one, one point right."
- When ruling an afterblow "too late," cite one of the five criteria in Section 1.3. Avoid the word "tempo."
- When ruling a hit "no quality," cite one of the criteria in Section 1.4.
- If you didn't see something or the exchange was too messy to call, say that instead of "no quality."
- When discussing what call to make with the judge, remain at your starting positions so fencers and spectators can hear. Keep discussions brief; if it is not quickly reconcilable, the exchange was unclear and should be thrown out.
...
**What is the difference between a double and an afterblow here?**
...
**Doesn't right of way encourage people to attack suicidally?**
We are playing a game in which there is no option of retreat; at some point someone has to attack. It is the defender's responsibility to attempt to prevent themselves from getting hit. If they instead choose to throw their own attack and the result is a double, the defender will not be rewarded. Target priority (legs lose) and the Hand layer (one hand loses) keep the riskiest attacks from being rewarded.
...
| :---- | :---- | :---- |
| **Spatulated** | Width of blade is widened AND distal thickness is thickened at tip. | None |
| **Thickened** | Distal thickness is thickened at tip, but blade is not widened. | Leather, bullet casing, or heat shrink plastic cap. Leather or bullet casing must widen the impact area at the tip. |
| **Rolled** | Tip is blunted by curving the tip over itself to create a loop. | Leather, tape, bullet casing, or heat shrink plastic cap. All edges of the rolled portion must be covered. |
| **Unmodified / Rounded** | Tip has the same width and distal thickness as the blade leading up to it. | Bullet casing or heat shrink plastic cap. |

- When using leather, only leather must be exposed at the point where the tip makes contact with the opponent.
- When using a bullet casing, only metal must be exposed at the point where the tip makes contact with the opponent.
